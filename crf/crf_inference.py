...
(1) https://github.com/asappresearch/structshot
(2) https://github.com/psunlpgroup/CONTaiNER
"""
...
def get_abstract_transitions(examples):
    """
    Compute abstract transitions on the training dataset for StructShot
    """
    tag_lists = [[event["event_type"] for event in example.event_list] for example in examples]
    sent_list = [example.sent for example in examples]
    print(len(tag_lists))

    s_o, s_i = 0., 0.
    o_o, o_i = 0., 0.
    i_o, i_i, x_y = 0., 0., 0.
    for tags, sent in zip(tag_lists, sent_list):
        if tags[0] == O_TOKEN: s_o += 1
        else: s_i += 1
...
class ViterbiDecoder:
    ...
    def __init__(self, n_tag, abstract_transitions, tau, device):
        """
        We assume the batch size is 1, so no need to worry about PAD for now
        n_tag: START, O, and I_Xs
        """
        super().__init__()
        self.transitions = self.project_target_transitions(n_tag, abstract_transitions, tau).to(device)
